...
const cache_1 = require("./ivy/cache");
const diagnostics_1 = require("./ivy/diagnostics");
const host_1 = require("./ivy/host");
const paths_1 = require("./ivy/paths");
const symbol_1 = require("./ivy/symbol");
const system_1 = require("./ivy/system");
const transformation_1 = require("./ivy/transformation");
/**
...
class Reproduction {
    ...
    apply(compiler) {
        ...
        for (const [key, value] of Object.entries(
            this.pluginOptions.fileReplacements
        )) {
    ...
    setupCompilation(compilation, state) {
        ...
        if (!state.resourceLoader) {
            state.resourceLoader = new resource_loader_1.WebpackResourceLoader(
                this.watchMode
            );
        }
        ...
        const diagnosticsReporter = (0,
        diagnostics_1.createDiagnosticsReporter)(compilation, (diagnostic) =>
        ...
        const system = (0, system_1.createWebpackSystem)(
            ...
            (0, paths_1.normalizePath)(compiler.context)
        );
        ...
        if (cache) {
            ...
            for (const changedFile of [
                ...
                const normalizedChangedFile = (0, paths_1.normalizePath)(
                    changedFile
                );
        ...
        (0, host_1.augmentHostWithDependencyCollection)(
            host,
        ...
        (0, host_1.augmentHostWithResources)(host, state.resourceLoader, {
            ...
            inlineStyleFileExtension:
                this.pluginOptions.inlineStyleFileExtension,
        });
        // Setup source file adjustment options
        (0, host_1.augmentHostWithReplacements)(
            host,
            this.pluginOptions.fileReplacements,
            moduleResolutionCache
        );
        (0, host_1.augmentHostWithSubstitutions)(
            host,
            this.pluginOptions.substitutions
        );
        // Create the file emitter used by the webpack loader
        const { fileEmitter, builder, internalFiles } = this.pluginOptions
            .jitMode
        ...
        for (const sourceFile of builder.getSourceFiles()) {
            ...
            compilation.fileDependencies.add(
                (0, paths_1.externalizePath)(sourceFile.fileName)
            );
            ...
            if (!sourceFile.isDeclarationFile) {
                currentUnused.add(
                    (0, paths_1.normalizePath)(sourceFile.fileName)
                );
            }
        }
        compilation.hooks.finishModules.tapPromise(
            ...
            async (modules) => {
                ...
                for (const webpackModule of modules) {
                    ...
                    if (resource) {
                        this.markResourceUsed(
                            (0, paths_1.normalizePath)(resource),
                            currentUnused
                        );
                    }
                }
    ...
    registerWithCompilation(compilation) {
        ...
        if (!fileEmitters) {
            ...
            compilation.compiler.webpack.NormalModule.getCompilationHooks(
                compilation
            ).loader.tap(PLUGIN_NAME, (context) => {
    ...
    markResourceUsed(normalizedResourcePath, currentUnused) {
        ...
        for (const dependency of dependencies) {
            this.markResourceUsed(
                (0, paths_1.normalizePath)(dependency),
                currentUnused
            );
        }
    }
    async rebuildRequiredFiles(modules, compilation, fileEmitter) {
        ...
        if (filesToRebuild.size > 0) {
            const rebuild = (webpackModule) =>
                new Promise((resolve) =>
                    compilation.rebuildModule(webpackModule, () => resolve())
                );
            ...
            for (const webpackModule of modules) {
                ...
                if (
                    resource &&
                    filesToRebuild.has((0, paths_1.normalizePath)(resource))
                ) {
                    modulesToRebuild.push(webpackModule);
                }
            }
            await Promise.all(
                modulesToRebuild.map((webpackModule) => rebuild(webpackModule))
            );
        }
    ...
    updateAotProgram(
        ...
        const transformers = (0, transformation_1.createAotTransformers)(
            builder,
            this.pluginOptions,
            exports.imageDomains
        );
            ...
            .then(() => {
                ...
                for (const sourceFile of builder.getSourceFiles()) {
                    ...
                    if (
                        ...
                        !angularCompiler.incrementalCompilation.safeToSkipEmit(
                            sourceFile
                        )
                    ) {
                        this.requiredFilesToEmit.add(
                            (0, paths_1.normalizePath)(sourceFile.fileName)
                        );
                        ...
                        const angularDiagnostics =
                            this.sourceFileCache.getAngularDiagnostics(
                                sourceFile
                            );
                ...
                return {
                    emitter: this.createFileEmitter(
                        builder,
                        (0, transformation_1.mergeTransformers)(
                        ...
                        (sourceFile) => {
                            ...
                            angularCompiler.incrementalCompilation.recordSuccessfulEmit(
                                sourceFile
                            );
                        }
                    ),
    ...
    updateJitProgram(compilerOptions, rootNames, host, diagnosticsReporter) {
        ...
        if (this.watchMode) {
            ...
            // When not in watch mode, the startup cost of the incremental analysis can be avoided by
            // using an abstract builder that only wraps a TypeScript program.
            builder = ts.createAbstractBuilder(
                rootNames,
                compilerOptions,
                host
            );
        ...
        diagnosticsReporter(diagnostics);
        const transformers = (0, transformation_1.createJitTransformers)(
            builder,
            this.compilerCliTooling,
            this.pluginOptions
        );
        return {
            fileEmitter: this.createFileEmitter(
                builder,
                transformers,
                () => []
            ),
    ...
    async addFileEmitHistory(filePath, content) {
        assert_1.strict.ok(
            this.webpackCreateHash,
            "File emitter is used prior to Webpack compilation"
        );
        const historyData = {
        ...
        if (this.webpackCache) {
            ...
            if (
                !history ||
                Buffer.compare(history.hash, historyData.hash) !== 0
            ) {
                // Hash doesn't match or item doesn't exist.
                await this.webpackCache.storePromise(
                    filePath,
                    null,
                    historyData
                );
